...
final class GameObjectTests: XCTestCase {
    func testAddChildSetsParent() {
        let parent = DefaultGameObject(name: "Parent")
    ...
    func testGlobalTransformCombinesParentAndChild() {
        let parent = DefaultGameObject(name: "Parent", transform: Transform2D(position: Vector2(x: 2, y: 2), scale: Vector2(x: 2, y: 2)))
        let child = DefaultGameObject(name: "Child", transform: Transform2D(position: Vector2(x: 1, y: 1)))
        parent.addChild(child)
